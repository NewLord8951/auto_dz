...
with sync_playwright() as p:
    ...
    # Обрабатываем задания в цикле, пока есть активные
    processed_count = 0
    max_attempts = 20  # Максимальное количество попыток чтобы избежать бесконечного цикла

    while processed_count < max_attempts:
        try:
            # Каждый раз заново ищем элементы после обновления DOM
                ...
                try:
                    ...
                    if upload_button:
                        ...
                        try:
                                ...
                                for submit_selector in submit_buttons:
                                    try:
                                        ...
                                        if submit_btn and submit_btn.is_visible():
                                            ...
                                            page.wait_for_timeout(5000)  # Ждем обработки отправки и обновления DOM
                                            ...
                                            try:
                                                ...
                                                logger.info("Модальное окно все еще открыто, пробуем закрыть")
                            ...
                            processed_count += 1
                            found_active = True
                            logger.success(f"Задание отправлено: {subject_name}")
                            break  # Прерываем цикл по заданиям после успешной обработки одного
            ...
            if not found_active:
                logger.info("Не найдено активных заданий для обработки")
                break
                
            # Ждем обновления DOM после отправки задания
            page.wait_for_timeout(3000)
    ...
    logger.info(f"Обработка домашних заданий завершена. Обработано: {processed_count}")
